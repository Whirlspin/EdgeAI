import { pricingPlan } from "../../utils/pricing-plan";
          ...
          {pricingPlan.map((plan, key) => (
            <div key={key} className="relative group h-full">
              <div className="bg-gradient-to-r from-blue-600 to-violet-600 p-1 rounded-3xl h-full">
                <div className="bg-box-bg border-box-border rounded-3xl shadow-lg shadow-box-shadow p-8 flex flex-col h-full relative">
                  {/* {plan.bestValue && (
                    <div className="absolute -top-3 left-1/2 -translate-x-1/2 bg-primary text-white text-xs font-bold uppercase tracking-wider px-3 py-1 rounded-full bg-gradient-to-tr from-blue-600 to-violet-600 bg-gr">
                      Best value
                    </div>
                  )} */}
                  ...
                  <ul className="mt-6 flex-1 space-y-3 text-left text-heading-3">
                    {plan.features.map((feature, keyfeatures) => (
                      <li className="flex items-center gap-2" key={keyfeatures}>
